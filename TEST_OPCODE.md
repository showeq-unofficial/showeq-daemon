...
> **Status (2026-07-05):** Test is now a build-time **backend on main** (`-DSEQ_TARGET=test`), not a standalone branch. This file is the ported work-of-record; the confirmed ids below were overlaid onto `conf/test/opcodes.toml` (Test-specific ids on a current-Live base — see commit "conf/test: overlay test-client's confirmed Test opcode ids"). Build/run via `./build.sh --target test` or `./up.py --build --wt test --target test`. The ids are a ~May-2026 Test snapshot — opcodes reshuffle each patch, so re-verify against a fresh Test capture (that re-verification IS the Test target's convergence-to-Live workflow). The "merge-back to main" framing below is historical (from when Test was the `test-client` branch).
