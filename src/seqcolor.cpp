...
SeqColor::SeqColor(const QString& spec)
{
  if (spec.isEmpty())
    return;
  ...
  if (spec.startsWith('#'))
  {
    ...
    if (spec.size() != 7)
      return;
    bool ok = false;
    uint32_t v = spec.mid(1).toUInt(&ok, 16);
    if (!ok)
      return;
    r = uint8_t((v >> 16) & 0xff);
    g = uint8_t((v >> 8) & 0xff);
  ...
  for (const auto& c : kNamedColors)
  {
    if (spec.compare(QLatin1String(c.name), Qt::CaseInsensitive) == 0)
    {
      r = c.r;
      g = c.g;
      b = c.b;
  ...
  // Unknown name: leave invalid.
}

QString SeqColor::name() const
{
